...
def centroid_calculator(centroid_list, file):
    ...
    if alien == 2 :
        ...
        return [['alien_a', 2, centroidx, centroidy]]
    elif alien == 3 :
        file.write(f"Organism Type: alien_b\n" + f"Centroid: {(centroidx,centroidy)}\n\n")
        return [['alien_b', 3, centroidx, centroidy]]
    elif alien == 4 :
        file.write(f"Organism Type: alien_c\n" + f"Centroid: {(centroidx,centroidy)}\n\n")
        return [['alien_c', 4, centroidx, centroidy]]
    elif alien == 5 :
        file.write(f"Organism Type: alien_d\n" + f"Centroid: {(centroidx,centroidy)}\n\n")
        return [['alien_d', 5, centroidx, centroidy]]
    elif alien >= 6 :
        ...
        if len(centroid_1) != 0:
            centroids.extend(centroid_calculator(centroid_1, file))

        if len(centroid_2) != 0:
            centroids.extend(centroid_calculator(centroid_2, file))

        if len(centroid_3) != 0:
            centroids.extend(centroid_calculator(centroid_3, file))

        if len(centroid_4) != 0:
            centroids.extend(centroid_calculator(centroid_4, file))
    return centroids
#----------------------------------------------------------------------------------------------
...
with open(txt_filename, "w") as file:
    centroids = centroid_calculator(centroid_list, file)
    ...
    for x , y in centroid_list:
        ...
        cv2.putText(img, f'Cluster of {len(centroid_list)} leds', (round(float(x)+40),round(float(y)-20)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1, cv2.LINE_AA)
        cv2.putText(img, f'Type: {centroids[0][0]} ', (round(float(x)+40),round(float(y))), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0, 255), 1, cv2.LINE_AA)
